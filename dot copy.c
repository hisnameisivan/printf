			...
			if (temp.znak == -1) /* num отрицательный, НЕТ флага '-', флаг '+' не важен */
			{
				if (flags->nul)
				{
					if (flags->dot)
						...
						ft_constructor(flags, temp, 5, new_num); /* ветка 6 */ /* точность не влияет */
				}
				else if (!(flags->nul))
				...
				if (flags->plus) /* num положительный, НЕТ флага '-' и ЕСТЬ флаг '+'  */
				{
					if (flags->nul)
					{
						if (flags->dot)
							...
							ft_constructor(flags, temp, 5, new_num); /* ветка 8 */  /* точность не влияет */
					}
					else if (!(flags->nul))
					...
					if (flags->nul)
					{
						...
						else if (!(flags->dot))
						{
							if (flags->space)
								...
								ft_constructor(flags, temp, 8, new_num); /* ветка 12 */
						}
					}
